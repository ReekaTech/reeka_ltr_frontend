...
export const propertyValidationSchema = Yup.object().shape({
  ...
  pricing: Yup.object().shape({
    base: Yup.number().min(0, 'Base price must be 0 or greater').required('Base price is required'),
    min: Yup.number().min(0, 'Minimum price must be 0 or greater').required('Minimum price is required'),
    max: Yup.number().min(0, 'Maximum price must be 0 or greater').required('Maximum price is required')
  })
});
...
export const leaseValidationSchema = Yup.object().shape({
  ...
  depositAmount: Yup.number().required('Deposit amount is required').min(0, 'Deposit amount must be positive'),
